			...
			try:
				...
				for filename in os.listdir("."):
					if filename.endswith(".dem"):
						os.rename(filename, "%s%s" % (str(counter), ".dem"))
						print "filename: %s" % filename
						counter = counter + 1	#add onto the counter
					elif filename.endswith("_num.tif"):
						print "elif %s" % filename
						os.remove(filename)
...
if len(IMGList) < 6:				#This will determine the number of rasters in your folder and return either statement
...
focalStat = FocalStatistics(Default_Hillshade, NbrRectangle(4,4, "CELL"), "MEDIAN", "DATA")  #This uses the Focal Statistics Tool and uses a moving 4x4 window to calculate the Median value and fill that in to a new raster
print "Focal Statistics Done"

arcpy.gp.Plus_sa(DEM_div_5, focalStat, "aerial_persp") #We are creating an Aerial Perspective Raster that looks like a lighter hillshade especially at the higher altitudes
